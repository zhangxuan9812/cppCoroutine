...
pid_t Thread::GetThreadId() {
  ...
  uint64_t tid;
// pthread_threadid_np is a non-standard function, but it is available on both Linux and macOS, which can be used to get
// the thread ID The first parameter is the thread, and the second parameter is the thread ID If you pass NULL as the
// first parameter, you can get the thread ID of the current thread If you pass a thread as the first parameter, you can
// get the thread ID of the specified thread If you are in linux, you can use syscall(SYS_gettid) to get the thread ID
#ifdef __APPLE__
...
#ifdef __APPLE__
  // pthread_setname_np is a non-standard function, but it is available on both Linux and macOS, which can be used to
  // set the name of the thread in order to facilitate debugging
