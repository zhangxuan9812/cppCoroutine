...
Fiber::Fiber() {
  ...
  m_ctx = boost::context::detail::make_fcontext(static_cast<char *>(m_stack) + m_stacksize, m_stacksize,
                                                &Fiber::empty_coroutine_function);
  if (!m_ctx) {
...
Fiber::Fiber(std::function<void()> cb, size_t stacksize, bool run_in_scheduler)
  ...
  m_ctx =
      boost::context::detail::make_fcontext(static_cast<char *>(m_stack) + m_stacksize, m_stacksize, &Fiber::MainFunc);
#else
  if (getcontext(&m_ctx)) {
    fmt::print(std::cerr, "Fiber(std::function<void()> cb, size_t stacksize, bool run_in_scheduler) failed\n");
    pthread_exit(NULL);
